...
- **Expanded LLM Provider Support**: Add compatibility for additional providers such as **WatsonX** and **vLLM**.  
- **Graph Database Connectivity**: Enable direct or batch loading of generated graphs into databases like **Neo4j**.  
- **`docling-graph query` Command**: Introduce a CLI command to query and interact with generated document graphs (e.g., via **LangChain**).  
- **Interactive Pydantic Model Generation**: Provide guided prompts for generating Pydantic templates directly from documents, following the detailed documentation guide.  
- **Flexible Component Instantiation**: Refactor initialization logic to allow user-defined components and easy replacement of default implementations.  
- **Optimized Batch Processing for Local GPU Inference**: Refine batching logic to improve performance during local GPU-based inference.  
- **Markdown Output Storage**: Add functionality to persist Markdown results from the Docling converter for reuse or reference.  