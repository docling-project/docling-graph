...
def convert_command(
    ...
    docling_pipeline: Annotated[Optional[str], typer.Option(
        "--docling-pipeline", "-d",
        help="Docling pipeline: 'ocr' or 'vision'."
    )] = None,
    
    # Docling export options Experiment
    export_docling_json: Annotated[bool, typer.Option(
        "--export-docling-json/--no-docling-json",
        help="Export Docling document as JSON."
    )] = True,
    
    export_markdown: Annotated[bool, typer.Option(
        "--export-markdown/--no-markdown",
        help="Export full document markdown."
    )] = True,
    
    export_per_page: Annotated[bool, typer.Option(
        "--export-per-page/--no-per-page",
        help="Export per-page markdown files."
    )] = False,
    ...
    final_export_docling_json = export_docling_json if export_docling_json is not None else docling_export_settings.get('docling_json', True)
    final_export_markdown = export_markdown if export_markdown is not None else docling_export_settings.get('markdown', True)
    final_export_per_page = export_per_page if export_per_page is not None else docling_export_settings.get('per_page_markdown', False)
