...
| `dense_skeleton_reconciliation` | `True` | One id-space LLM call after skeleton merge that collapses same-entity aliases discovered at different granularities across batches. |
| `parallel_workers` | `1` | When > 1, Phase 1 skeleton batches and Phase 2 fill batches run in parallel to reduce wall-clock time. |
| `max_pass_retries` | `1` | Retries per LLM call when output validation fails. |
