...
- **Model as an entity only what the document names.** If instances have no verbatim label the model can copy (an unnamed "study", "experiment", "run"), make it a component or give it a *derived* deterministic id — never a required free-text id the model has to invent.
...
- Class docstrings **front-load the discriminating sentence in the first ~240 chars**: what this entity IS, what it is NOT (the dominant confusable), any small known cardinality, and any sibling-path routing rule. Everything past the cut reaches only Phase 2. The pipeline logs a `Semantic guide: class docstring truncated` warning naming any class whose docstring overruns the window — heed it.
- A field is a **liability if the benchmark ground truth does not model it** (`omit when …` conditionals the model ignores at discovery, fields the corpus abandoned): it becomes pure false-positive noise. Template and GT co-evolve — remove the field or extend the GT.
...
- The **docstring's opening sentence decides classification.** Phase 1 places instances at paths using these descriptions ("an Option is not an Offre", "one clause per Exclusion, never one per keyword", "categories, not enumerated elements"). Front-load, in the first ~240 chars: what the entity is; the **dominant confusable it is NOT** ("geographic regions and revenue sub-lines are NOT reportable segments"); any **small known cardinality** ("the 3–6 reportable segments the segment note names") — a bound the model will otherwise ignore, minting one instance per table row; and any **sibling-path routing rule** ("a common exclusion goes in the root `exclusions_communes`, not under a garantie"), because the model chooses the path at discovery. Anything past ~240 chars never reaches Phase 1. The build logs `Semantic guide: class docstring truncated at 240 chars for <path>` for any class that overruns the window — treat that warning as "your discovery-time steering silently fell off the cliff; move it earlier."
...
3. **Verbatim-copyable, never invented — and if it can't be, it isn't an entity.** The id must be a label the model can copy from the page: a code, a proper name, a figure/table label. Never instruct fallback generation (`"ITEM-1"`, `"Offer 2"`, `"STUDY-BINDER-MW"`): invented ids differ between batches, so nothing matches, and positional labels defeat the skeleton's preference for the most specific proper name. The deeper rule (**name-ability**): if the document does not *name* the instances of a class at all — an unlabeled "study", "experiment", or "run" that exists only as prose structure — then it should not be an identity-bearing entity. Make it a component (filled inline with its named parent), collapse it into the nearest named level, or give it a **derived** deterministic id (a figure/table reference, or a parent-scoped positional key). A required free-text id on an unnamed thing is unwinnable by construction: the extraction invents one id, the ground truth invents another, and strict matching can never succeed.
...
1. **Duplicate instances merge fill-missing-only.** When the same entity is discovered at several paths (e.g. one person in both the board list and the executive list), all instances collapse into one node: later instances fill attributes the first left empty, but **conflicting non-empty values keep the first-seen value**. So when one real-world thing plays several roles with role-specific data, keep the shared entity **identity-minimal** (its id field and nothing else) and model each role as its own entity — identified by the same name, holding that role's fields, linking to the shared node via an edge:
    ...
    Role fields on the shared node would resolve arbitrarily (whichever list was processed first wins); per-role entities never conflict, and dense extraction discovers each role as its own catalog instance. The `reference=True` on the shared-identity link means the Person ref is emitted by the role's own fill call — an identity-only node needs no discovery of its own. When two role lists are drawn from *different* document sections (a board-of-directors slate vs an executive-officers roster), state that they are **disjoint** in each docstring ("the directors list only; executive officers are not board members unless the report seats them on the board") and name the source section of each ("EVERY officer in the *Information about our Executive Officers* item") — otherwise the model pools everyone into whichever role it discovers first, and the under-populated role's recall collapses.
...
- **Say when a summary field is derivable.** A short field that condenses a longer sibling being filled in the same call (a `resume` of a `texte`, a `summary` of a `description`) is left empty by default — the model treats "not stated verbatim" as "omit." State in its description that it is *derivable from* the long field ("one-line condensation of `texte`; never leave empty when `texte` is filled"). The fill prompt already permits this globally (a condensation of an extracted field is a valid fill, not an invention), but the per-field cue is what the model acts on. This matters beyond cosmetics: the structural-alignment rung pairs synthetic-id nodes by attribute overlap, so a sparse summary field costs matchability, not just display quality.
...
- **Entity classes the document never names** given a required free-text id (the model and the ground truth invent different ids; strict matching is unwinnable — make it a component or use a derived id).
...
- **Negative `"omit when X"` instructions relied on at discovery** — the model ignores them and mints the class anyway (a `RiskFactor` class kept "in case" produces false positives on every document that references risks externally). Delete the class from the corpus variant instead.
- **Fields the benchmark ground truth does not model** — pure false-positive noise; keep template and GT in sync.
