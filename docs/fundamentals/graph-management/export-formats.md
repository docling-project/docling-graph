...
| **Cypher** (`graph.cypher`) | A string node property containing escaped JSON, e.g. `SET n.__provenance__ = "{\"match\": \"verbatim\", ...}"`. |
...
// Idempotent: safe to re-run (nodes MERGEd on their id).
...
// --- Nodes ---
MERGE (n:Invoice {id: "Invoice_a1b2c3d4e5f6"})
SET n.invoice_number = "INV-001", n.total = 1000.0;
MERGE (n:Organization {id: "Organization_b2c3d4e5f6a1"})
SET n.name = "Acme Corp";
MERGE (n:Address {id: "Address_c3d4e5f6a1b2"})
SET n.street = "123 Main St", n.city = "Paris";

// --- Relationships ---
MATCH (a:Invoice {id: "Invoice_a1b2c3d4e5f6"}), (b:Organization {id: "Organization_b2c3d4e5f6a1"})
MERGE (a)-[:ISSUED_BY]->(b);
MATCH (a:Organization {id: "Organization_b2c3d4e5f6a1"}), (b:Address {id: "Address_c3d4e5f6a1b2"})
MERGE (a)-[:LOCATED_AT]->(b);
```

Every statement is self-contained and `;`-terminated, so the file can be piped
straight through `cypher-shell`. Property values keep their types (numbers,
booleans and lists become Cypher literals; dicts such as `__provenance__`
become JSON strings). Because nodes and relationships are `MERGE`d on identity
only, re-running the script updates properties in place instead of duplicating
the graph, and the per-label uniqueness constraints keep every lookup
index-backed.

For a one-shot bulk load into an empty database, `CypherExporter(style="create")`
emits plain `CREATE` statements instead; re-running such a script trips the
uniqueness constraints rather than silently duplicating data.
...
# Export to Cypher — style="merge" (default) is idempotent;
# style="create" emits plain CREATE statements for one-shot loads.
