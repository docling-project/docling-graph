...
class Issuer(BaseModel):
    ...
    name: str = Field(
        ...
        examples=["Robert Schneider AG", "Slack"]
    )
...
class Client(BaseModel):
    ...
    email: Optional[str] = Field(
        ...
        examples=["client@client.com"]
    )
    website: Optional[str] = Field(
        default=None,
        description="Client website URL",
        examples=["www.client.ch"]
    )
...
class Invoice(BaseModel):
    ...
    bill_no: str = Field(
        ...
        examples=["3139", "1223113"]
    )
    date: str = Field( # Kept as string to match your old model, use date for validation
